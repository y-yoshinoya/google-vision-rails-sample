module Crud
  module ApplicationHelper
    ...
    def icon_tag(icon_class, **options)
      ...
      right = tag.span right, class: "d-none d-lg-inline-block" if responsive
        ...
        if can?(action, resource)
          ...
          if block
            ...
            if options.delete(:tooltip)
              ...
              label = crud_icon_tag(action, responsive: options[:responsive])
            else
              label = crud_icon_tag(action, label: label, responsive: options[:responsive])
            end
    ...
    def crud_table(columns, resources, actions, options = nil)
      ...
      options[:class] ||= "table table-hover table-xm crud-table mb-0"
    ...
    def modal_selector(options)
      content_tag(:div, class: "form-check custom-control #{options[:type] == "checkbox" ? "custom-checkbox" : "custom-radio"}") do
        content_tag(:input, nil, options.merge(class: "custom-control-input")) +
        content_tag(:label, " ", class: "custom-control-label", for: options[:id])
      end
    end
    ...
    def advanced_search_input(f, column)
      ...
      column_size_class = @remote ? "col-3" : "col-2"
      ...
      tag.div class: "advanced-search-input" do
        ...
        tag.div(class: "form-row d-none") do
          concat tag.div(search_operator_select("op[#{column}]", operators, selected_operator), class: "col-5")
          if args = SearchQuery::Operator[selected_operator].try(:args)
            (0...args).each do |i|
              input_options = options.deep_merge(
                input_html: { name: "v[#{column}][]", class: "form-control rounded-0" },
                wrapper_html: { class: "col-7" }
              )
    ...
    def search_operator_select(name, operators, selected)
      ...
      select_tag(name, options_for_select(options, selected), class: "operator form-control form-control-xm rounded-0", include_blank: true)
    end
